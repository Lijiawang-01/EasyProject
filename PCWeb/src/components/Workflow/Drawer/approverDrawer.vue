<template>
	...
	>
		<div class="demo-drawer__content">
				...
				<div class="approver_content">
					...
					<p class="selected_list" v-if="approverConfig.settype == 1">
						<span v-for="(item, index) in approverConfig.nodeUserList" :key="index"
							>{{ item.name }}
							<img src="@/assets/images/add-close1.png" @click="removeEle2(approverConfig.nodeUserList, item)" />
						</span>
				...
				<div class="approver_manager" v-if="approverConfig.settype == 2">
					<p>
						...
						<select v-model="approverConfig.directorLevel">
							<option v-for="item in props.directorMaxLevel" :value="item" :key="item">{{ item == 1 ? '直接' : '第' + item + '级' }}主管</option>
						</select>
					</p>
				...
				<div class="approver_manager" v-if="approverConfig.settype == 7">
					...
					<p style="padding-bottom: 20px">
						...
						<select v-model="approverConfig.examineEndDirectorLevel">
							<option v-for="item in props.directorMaxLevel" :value="item" :key="item">{{ item == 1 ? '最高' : '第' + item }}层级主管</option>
						</select>
					</p>
				</div>
				<div
			...
			<employeesDialog v-model:visible="approverVisible" :data="checkedList" @change="sureApprover" :activeNames="1" />
			<employeesDialog v-model:visible="approverRoleVisible" :data="checkedRoleList" @change="sureRoleApprover" :activeNames="2" />
		</div>
	</el-drawer>
</template>
<script lang="ts" setup>
import { watch, computed, ref } from 'vue';
import { setApproverStr, removeEle2 } from '@/utils/workflowIndex';
import { setTypes, selectModes, selectRanges } from '@/utils/workflowConst';
import workflowStore from '@/store/workflow';
import employeesDialog from '../dialog/selectDialog.vue';
import { NodeConfig, NodeUserList } from '@/class/workflow';
...
let approverConfig = ref({} as NodeConfig);
let approverVisible = ref(false);
let approverRoleVisible = ref(false);
let checkedRoleList = ref([] as NodeUserList[]);
...
const saveApprover = () => {
	...
	approverConfig.value.flag = true;
	setApproverConfig(approverConfig.value);
	closeDrawer();
};
const closeDrawer = () => {
	setApprover(false);
