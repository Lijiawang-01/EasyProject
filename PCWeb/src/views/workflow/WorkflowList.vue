<template>
	<el-card class="box-card">
		...
		<el-table :data="tableData" style="width: 100%" ref="multipleTableRef" row-key="id">
			...
			<el-table-column label="名称" width="180">
				<template #default="scope">
					<el-popover effect="light" trigger="hover" placement="top" width="auto">
						<template #default>
							<div>address: {{ scope.row.workflowTemplateTitle }}</div>
						</template>
					</el-popover>
				</template>
			</el-table-column>
			<el-table-column label="描述" width="300">
				<template #default="scope">
					<div>{{ scope.row.workflowTemplateDesc }}</div>
				</template>
			</el-table-column>
			...
			<el-table-column label="是否启用">
				<template #default="scope">
					<div>{{ scope.row.activeStatus }}</div>
				</template>
			</el-table-column>
	...
	<setVue :setVisible="setVisible" :flowId="flowId" @CloseSet="CloseSet"></setVue>
</template>
...
import setVue from './WorkflowSet.vue';
import { WorkflowTmpModel } from './class/WorkflowTmpModel';
import { getTemplateData, getTemplateInfo } from '@/api/workflow/workflowApi';
...
const handleSet = (_index: number, row: WorkflowTmpModel) => {
	flowId.value = row.id;
...
const handleEdit = (_index: number, row: WorkflowTmpModel) => {
	info.value = JSON.stringify(row);
	addVisible.value = true;
};

//单个删除
const handleDelete = async (_index: number, row: WorkflowTmpModel) => {
...
//表格
const tableData = ref<Array<WorkflowTmpModel>>();
onMounted(async () => {
	LoadTableData();
});
const LoadTableData = async () => {
	...
	getTemplateData(parms)
		.then((res) => {
			form.total = (res as any).total;
			tableData.value = (res as any).data as WorkflowTmpModel[];
		})
		.catch((err) => {
			ElMessage({
				message: err,
